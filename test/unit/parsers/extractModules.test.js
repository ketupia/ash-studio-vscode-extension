...
defmodule MyApp.User do
  ...
  attributes do
    ...
    assert.strictEqual(result[0].section, 'attributes', 'Should find attributes section');
    assert.strictEqual(result[0].details.length, 2, 'Should find two attribute details');
    assert.strictEqual(result[0].details[0].name, 'name', 'First attribute should be name');
    assert.strictEqual(result[0].details[1].name, 'email', 'Second attribute should be email');
  });
...
defmodule MyApp.User do
  ...
  actions do
    ...
    assert.strictEqual(attributesSection.details.length, 1, 'Should find one attribute');
    assert.strictEqual(attributesSection.details[0].name, 'name', 'Attribute should be name');
    ...
    assert.strictEqual(actionsSection.details.length, 2, 'Should find two actions');
    assert.strictEqual(actionsSection.details[0].name, 'register', 'First action should be register');
    assert.strictEqual(actionsSection.details[1].name, 'update_profile', 'Second action should be update_profile');
  });
...
defmodule MyApp.User do
  ...
  attributes do
    ...
    assert.strictEqual(result.length, 1, 'Should find one section');
    assert.strictEqual(result[0].section, 'attributes', 'Should find attributes section');
    assert.strictEqual(result[0].details.length, 2, 'Should find two attributes');
    assert.strictEqual(result[0].details[0].name, 'name', 'First attribute should be name');
    assert.strictEqual(result[0].details[1].name, 'email', 'Second attribute should be email');
  });
...
defmodule MyApp.Album do
  ...
  postgres do
    ...
    assert.deepStrictEqual(sectionNames, ['actions', 'attributes', 'postgres'], 'Should find all expected sections');
    
    // Check attributes section has multiple children
    const attributesSection = result.find(s => s.section === 'attributes');
    assert.ok(attributesSection.details.length >= 2, 'Should find multiple attributes');
  });
...
defmodule MyApp.User do
  ...
  actions do
    ...
    const attributesSection = result.find(s => s.section === 'attributes');
    assert.strictEqual(attributesSection.details.length, 1, 'Should find exactly one attribute');
    
    const actionsSection = result.find(s => s.section === 'actions');
    assert.strictEqual(actionsSection.details.length, 1, 'Should find exactly one action');
  });
...
defmodule MyApp.TestResource do
  ...
  actions do
    ...
    const attributeDetails = attributesSection.details.filter(d => d.detail === 'attribute');
    assert.strictEqual(attributeDetails.length, 1, 'Should find one attribute');
    assert.strictEqual(attributeDetails[0].name, 'name', 'Attribute should be name');
    ...
    const createDetails = actionsSection.details.filter(d => d.detail === 'create');
    assert.strictEqual(createDetails.length, 1, 'Should find one create action');
    assert.strictEqual(createDetails[0].name, 'register', 'Create action should be register');    
  });
});
