...
const AshResourceConfig = require('../../../../dist/src/parsers/configurationDriven/AshResource.config').default;
...
describe('Recursive DSL Parsing', () => {
  it('should correctly parse resource with actual Ash configuration', () => {
    ...
    assert.strictEqual(attributesSection.details.length, 2, 'Should have 2 attribute details');
    ...
    assert.strictEqual(actionsSection.details.length, 5, 'Should have 5 action details');
  ...
  it('should handle nested DSL blocks with custom configurations', () => {
    ...
    assert.strictEqual(attributesSection.section, 'attributes', 'Should be an attributes section');
    ...
    assert.strictEqual(attributesSection.details.length, 1, 
      'Should find the attribute (nested blocks require explicit configuration)');
    
    // Verify specific details
    const attributeDetail = attributesSection.details.find(d => d.detail === 'attribute');
    assert.ok(attributeDetail, 'Should find attribute detail');
    assert.strictEqual(attributeDetail.name, 'email', 'Should extract attribute name');
  });
});
