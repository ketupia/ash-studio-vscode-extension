...
const mockTextDocument = (text) => ({
  ...
  lineAt: (lineIndex) => ({
    ...
    range: { start: { line: lineIndex, character: 0 }, end: { line: lineIndex, character: text.split('\n')[lineIndex].length } }
  }),
...
describe('Parser Service Integration', () => {
  it('should parse Ash resource using configuration-driven parser', async () => {
    ...
    assert.strictEqual(resourceModule.type, 'Ash.Resource', 'Should identify as an Ash Resource');
    
    // Verify sections were found
    assert.ok(resourceModule.sections, 'Should have sections');
    assert.ok(resourceModule.sections.length >= 2, 'Should have at least attributes and actions sections');
    
    // Find attributes section
    const attributesSection = resourceModule.sections.find(s => s.section === 'attributes');
    assert.ok(attributesSection, 'Should have an attributes section');
    assert.strictEqual(attributesSection.details.length, 2, 'Should have 2 attributes');
    
    // Find actions section
    const actionsSection = resourceModule.sections.find(s => s.section === 'actions');
    assert.ok(actionsSection, 'Should have an actions section');
    assert.strictEqual(actionsSection.details.length, 1, 'Should have 1 action');
    ...
    assert.strictEqual(nameAttr.detail, 'attribute', 'Should be an attribute detail');
    
    // Verify action details
    const registerAction = actionsSection.details.find(d => d.name === 'register');
    assert.ok(registerAction, 'Should have register action');
    assert.strictEqual(registerAction.detail, 'create', 'Should be a create action');
  });
  ...
  it('should handle resources with nested blocks', async () => {
    ...
    assert.strictEqual(attributesSection.details.length, 1, 'Should have 1 attribute');
    assert.strictEqual(actionsSection.details.length, 1, 'Should have 1 action');
