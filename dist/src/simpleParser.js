...
function parseAshDocumentSimple(document) {
    ...
    if (isAshType && !isAshResource) {
        try {
            ...
            errors.push({
                message: `Failed to parse Ash type definition: ${error instanceof Error ? error.message : String(error)}`,
                line: 0,
                column: 0,
                offset: 0,
            });
            ...
            try {
                ...
                for (let j = i + 1; j < lines.length && blockDepth > 0; j++) {
                    ...
                    const doMatches = (currentLine.match(REGEX_PATTERNS.doKeyword) || [])
                        .length;
                    const endMatches = (currentLine.match(REGEX_PATTERNS.endKeyword) || []).length;
                ...
                errors.push({
                    message: `Failed to parse section '${sectionName}' on line ${i + 1}: ${error instanceof Error ? error.message : String(error)}`,
                    line: i,
                    column: startColumn,
                    offset: 0,
                });
...
function parseInnerMacros(lines, startLine, endLine, errors) {
    const children = [];
    for (let i = startLine + 1; i < endLine; i++) {
        ...
        try {
            ...
            errors.push({
                message: `Failed to parse macro on line ${i + 1}: ${error instanceof Error ? error.message : String(error)}`,
                line: i,
                column: 0,
                offset: 0,
            });
